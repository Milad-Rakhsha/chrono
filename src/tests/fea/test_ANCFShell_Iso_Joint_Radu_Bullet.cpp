...
int main(int argc, char* argv[]) {
    ...
    int numFlexBody = 1;
    const int num_steps = 5000;        // Number of time steps for unit test (range 1 to 4000)
    ...
    ChSharedPtr<ChNodeFEAxyzD> NodeFirst(my_mesh->GetNode(0).DynamicCastTo<ChNodeFEAxyzD>());
...
// Defining the Body 1
	ChSharedPtr<ChBody> BGround(new ChBody);
	...
	BGround->SetPos(ChVector<>(0, 0, 0)); // Y = -1m
...
// Defining the Body 3
	...
	my_link_12->Initialize(BGround, Body_2, ChCoordsys<>(ChVector<>(-2.0, 0, 0), Q_from_AngX(3.14159265358979/2.0)));
	my_system.AddLink(my_link_12);

	// Add another revolute joint
	// .. a revolute joint between flywheel and rod
	ChSharedPtr<ChLinkLockRevolute> my_link_23(new ChLinkLockRevolute);
	//my_link_23->Initialize(Body_2, Body_3, ChCoordsys<>(ChVector<>(-1E-12, 0, 0), Q_from_AngX(3.14159265358979 / 2.0)));
	my_link_23->Initialize(Body_2, Body_3, ChCoordsys<>(ChVector<>(0, 0, 0), Q_from_AngX(3.14159265358979 / 2.0)));
...
// Constraining a node to the truss
    ...
    std::ifstream file2("TestJoint_RB1.txt");
	...
	// std::cout << "NodeFirst->pos.z = " << NodeFirst->pos.z << "\n";
	 std::cout << "Body_2->coord.pos.x " << Body_2->coord.pos.x << "\n";
	 std::cout << "Body_2->coord.pos.y " << Body_2->coord.pos.y << "\n";
	 std::cout << "Body_2->coord.pos.z " << Body_2->coord.pos.z << "\n";
	...
	csv << m_data[0][it] << m_data[1][it] << m_data[2][it] << m_data[3][it] <<
		m_data[4][it] << m_data[5][it] << m_data[6][it] << std::endl;
    csv.write_to_file("TestJoint_RB1.txt"); 
	}

...
double fyDim = byDim;
double fzDim = 2;
double cyl_length = bzDim;
double cyl_radius = .3;
int cylinder_id;
ChVector<> hdim(bxDim / 2, byDim / 2 + 1.0 * particle_dist, bzDim / 2);
ChVector<> fluidDim(fxDim / 2 + 0.5 * particle_dist, fyDim / 2 + 1.0 * particle_dist, fzDim / 2 + 0.5 * particle_dist);
ChVector<> tankInitPos(hdim.x() - fluidDim.x(), 0.5 * particle_dist, -fzDim / 2 + 0.12 * particle_dist);
...
const std::string out_dir = "DamBreak_w_cylinder";
...
double massCalculator(double Kernel_h, double InitialSpacing) {
    int IDX = 3;
    real h = Kernel_h;
    ...
    for (int i = -IDX; i <= IDX; i++)
        for (int j = -IDX; j <= IDX; j++)
            for (int k = -IDX; k <= IDX; k++) {
                ChVector<> pos(i * InitialSpacing, j * InitialSpacing, k * InitialSpacing);
                real dist = pos.Length();
                double W3 = KPOLY6;
                if (dist < Kernel_h) {
                    count++;
                    sum_wij += W3;
...
void AddTank(ChSystemParallelNSC* sys) {
    ...
    ChVector<> cyl_pos = ChVector<>(0.0 + bxDim / 2, 0, tankInitPos.z() + bzDim / 2);

    cylinder->SetPos(cyl_pos + ChVector<>(0, 0.0, 0.0));
    cylinder->SetBodyFixed(true);
    ...
    utils::AddCylinderGeometry(cylinder.get(), cyl_radius, cyl_length / 2, ChVector<>(0), cyl_rot);
...
void AddFluid(ChSystemParallelNSC* sys) {
    ...
    Mass_Constant = massCalculator(mKernel_radius, particle_dist);
...
int main(int argc, char* argv[]) {
    ...
    std::ofstream ForceFile(demo_dir + "/KCSPH.txt");
    ForceFile << "Time,Fx,Fy,Fz\n";
    ForceFile.close();
    ...
    while (time < time_end) {
        ...
        real3 Force = fluidSystem->GetBodyContactForce(cylinder->GetId());

        printf("time %f time_end %f, Force=%f,%f,%f\n", time, time_end, Force.x, Force.y, Force.z);

        std::ofstream ForceFile(demo_dir + "/KCSPH.txt", std::ios::app);
        ForceFile << time << "," << Force.x << "," << Force.y << "," << Force.z << std::endl;
        ForceFile.close();
